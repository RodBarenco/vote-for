...
function VotingPolls() {
  ...
  useEffect(() => {
    const fetchParticipatingPolls = async () => {
      try {
        const pollsRef = collection(db, 'poll');
        const q = query(pollsRef, where(`participants.${user.id}`, '!=', null));
        const querySnapshot = await getDocs(q);
        const participatingPollsData = [];
  
        querySnapshot.forEach((doc) => {
          const pollData = doc.data();
          if (pollData.participants && pollData.participants[user.id]) {
            participatingPollsData.push({
              id: doc.id,
        ...
        setParticipatingPolls(participatingPollsData);
        console.log(participatingPollsData);
      } catch (error) {
        console.error('Error fetching participating polls:', error);
      }
    };
  
    if (user) {
      fetchParticipatingPolls();
    }
  }, [user]);
  

  const handleVoteClick = (pollId, password) => {
    navigate(`/poll/${pollId}?passforvote=${password}`);
  };
  ...
  return (
    <div className="bg-global-gradient flex flex-col items-center h-screen p-8 text-white text-sm border-2 overflow-x-hidden">
      ...
      <h1 className="font-semibold text-title text-3xl">Votações que participo</h1>
      {participatingPolls.map((poll) => (
        <div key={poll.id} className="bg-slate-950 text-white p-4 m-4 rounded-xl border-2 w-4/5">
          ...
          <button
            ...
            onClick={() => handleVoteClick(poll.id, poll.password)}
          >
            Entrar
          </button>
        </div>
